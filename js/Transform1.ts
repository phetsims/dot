...
class Transform1 {
  ...
  public inverse( x: number ): number {
    ...
    affirm( approxEquals( this.evaluationFunction( result ), x ), `Incorrect evaluation for x=${x}` );
    return result;
  }
}
